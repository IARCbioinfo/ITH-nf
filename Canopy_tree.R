...
##
patient_id = args[1]
output_Canopy  = args[2]
lib_path   = args[3]

##
cat("####### ARGUMENTS #######\n")
cat(paste("patient_id: ", patient_id, "\n", sep=''))
cat(paste("Canopy_output: ", output_Canopy, "\n", sep=''))
...
bics = bicsn = c()
for (K in 2:9) {
  bic_tmp = try(read.table(file = output_Canopy), silent = TRUE)
...
## Plot K selection
##########################################
svg(filename=paste(output_Canopy, patient_id, ".optimal_K.svg", sep=''))
...
load(file = paste(output_Canopy, patient_id, ".K", optimalBIC$K, ".postmcmc_image.rda", sep=''))
post = canopy.post(sampchain = sampchain,
                   projectname = output_Canopy,
                   K = 1,
                   numchain = numchain,
                   burnin = 90,
                   thin = 5,
...
custom_canopy.plottree(output.tree, pdf=TRUE, pdf.name = paste(output_Canopy, patient_id, '_TREE_most_likely.pdf', sep=''), txt = TRUE, txt.name = paste(output_Canopy, patient_id, '_TREE_most_likely.txt', sep=''))